...
const Today = () => {
	...
	const [freeService, paidSales, totalSales, { delivery_charge, card_charge, e_coupon_charge, e_pay_charge, cash_charge, paid_point, hd_coupon_charge, etc_delivery_charge, fran_coupon_charge, bana_point, hd_coupon_charge_2 }] = useMemo(() => {
		// 매출 상세 내역
		let todayData = {
			delivery_charge: 0, 
			card_charge: 0, 
		...
		if (data) {
			...
			paidSales = data[0].card_charge + data[0].cash_charge + data[0].etc_delivery_charge + data[0].e_coupon_charge + data[0].hd_coupon_charge + data[0].paid_point;
	...
	return (
		<tr>
			...
			<td>
				{Utils.numberComma(e_pay_charge || 0)}원<span className='percentage'>({(100 * e_pay_charge/totalSales || 0).toFixed(1)}%)</span>
			</td>
			<td>
				{Utils.numberComma(cash_charge || 0)}원<span className='percentage'>({(100 * cash_charge/totalSales || 0).toFixed(1)}%)</span>
			</td>
			<td>
				{Utils.numberComma(paid_point || 0)}원<span className='percentage'>({(100 * (paid_point + e_coupon_charge)/totalSales || 0).toFixed(1)}%)</span>
			</td>
			<td>
				{Utils.numberComma(hd_coupon_charge || 0)}원<span className='percentage'>({(100 * hd_coupon_charge/totalSales || 0).toFixed(1)}%)</span>
			</td>
			<td>
				{Utils.numberComma(e_coupon_charge || 0)}원<span className='percentage'>({(100 * e_coupon_charge/totalSales || 0).toFixed(1)}%)</span>
			</td>
			<td>
