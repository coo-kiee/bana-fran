...
const SideMenu: React.FC<{
  activeMenu: SideMenuBarType;
  sideMenus: Array<SIDE_MENU_TYPE>;
  setActiveMenu: React.Dispatch<React.SetStateAction<SideMenuBarType>>;
}> = ({ activeMenu, sideMenus, setActiveMenu }) => {
      ...
      {sideMenus.map((data: SIDE_MENU_TYPE, key) => {
        const childMenu = data.child?.map((subData, ckey) => (
          <li key={`sublink_${ckey}`} className={`${activeMenu.subIndex === subData.id ? 'active' : ''}`}>
            <Link to={data.path + subData.path} className="menu" onClick={() => handleSubMenuClick(subData.id)}>
              {subData.name}
            </Link>
          </li>
        ));
        ...
        return childMenu.length > 0 ? (
          ...
          >
            <span className="menu" onClick={() => handleMenuClick(data.id, data.id !== activeMenu.index)}>
              {data.name}
            </span>
            <ul className="depth-list">{childMenu}</ul>
          </li>
        ) : (
          <li key={`link_${key}`} className={`list-item ${activeMenu.index === data.id ? 'active' : ''}`}>
            <Link to={data.path} className="menu" onClick={() => handleMenuClick(data.id)}>
              {data.name}
            </Link>
          </li>
        );
...
const SideMenubar: React.FC = () => {
  ...
  // 매장 변경.
  const handleChargeFranCode = useCallback(
  ...
  return (
    <nav>
      ...
      <div className="user">
        <p>
          <span className="name">{localStorage.getItem('userNm')}</span>님
        </p>
        <p>안녕하세요.</p>
      </div>
      ...
      {/* 로그아웃 */}
      <div className="user-function-layout">
        <div className="user-function">
          ...
          <p>
            <Link to={'/password_change'}>비밀번호 변경</Link>
          </p>
        </div>
      </div>
    </nav>
