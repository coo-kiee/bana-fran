...
// Type
import { SearchInfoType, SearchInfoSelectType, SearchInfoRadioType, isSelect, isRadio, OPTION_TYPE } from 'types/etc/etcType';
 ...
 */
const CalanderSearch: React.FC<CalendarSearchProps> = ({ title, dateTitle, dateType, searchInfo, setSearchInfo, optionType, selectOption, radioOption, optionList, handleSearch, ...datePickerProps }) => {
    ...
    const handleSearchRadio = (searchType: string) => {
        return setSearchInfo({
            from: format(new Date(searchInfo.from), searchType === 'M' ? 'yyyy-MM' : 'yyyy-MM-dd'), 
            to: format(new Date(searchInfo.to), searchType === 'M' ? 'yyyy-MM' : 'yyyy-MM-dd'),
            searchType
 ...
 */
type ReactDatePickerOptionProps = Pick<ReactDatePickerProps, 'minDate' | 'showMonthYearPicker' | 'showFullMonthYearPicker'>
/**
